...
export class UpdatePNAccessoryComponent implements OnInit {
  ...
  ngOnSubmit(): void {
    ...
    this._appService.uploadFile(this.ADVideo).subscribe((response: any) => {
      ...
      if (response.fileStorageID != null || undefined) {
        ...
        this._pNAccessoriesService.updatePNAccessory(pNAccessory).subscribe((response: any) => {
          ...
          if (response != null || response != undefined) {
            ...
            if (response) {
              this._router.navigate([`/pNAccessories/view/${response}`])
            }
          }
        });
      }
  ...
  generateAIContent(name: string, specs: string) {
    this._appService.generateAIContent({
      prompt: `generate phone ad description of ${name} based on ${specs} format result as text.`
