...
import { ENAppliance } from 'src/app/eNAppliances/eNAppliance';

import { ENAppliancesService } from 'src/app/eNAppliances/eNAppliances.service';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-update-enappliance',
  templateUrl: './updateENAppliance.component.html',
  styleUrl: './updateENAppliance.component.scss'
})
export class UpdateENApplianceComponent implements OnInit {
  ...
  updateENAppliancesForm = this._formBuilder.group({
  ...
  ngOnSubmit(): void {
    const formValues = this.updateENAppliancesForm.value;
    ...
    this._appService.uploadFile(this.ADVideo).subscribe((response: any) => {
      ...
      if (response.fileStorageID != null || undefined) {
        ...
        this._ENAppliancesService.updateENAppliance(eNAppliance).subscribe((response: any) => {
          ...
          if (response != null || response != undefined) {
            ...
            if (response) {
              this._router.navigate([`/eNAppliances/view/${response}`])
            }
          }
        });
  ...
  resetForm(): void {
    this.updateENAppliancesForm.reset();
  }
 ...
 generateAIContent(name: string) {
    this._appService.generateAIContent({
      prompt: `generate description electrical appliance of ${name} format result as text.`
